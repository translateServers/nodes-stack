...
- [ ] **5.4 Sheet 快捷键分层**
  - 结果：Sheet 打开期间画布全局快捷键挂起；Sheet 内 Ctrl+Z/Ctrl+Shift+Z 走全局本地编辑历史；Esc 分层（取消连线 → 取消选择 → 关闭）。
  - 验证：组件测试覆盖快捷键作用域切换与 Esc 分层顺序；画布快捷键在 Sheet 关闭后恢复。
  - 依赖：4.7、5.2。
...
- [ ] **8.1 沙盒运行时与模拟触发**
  - 结果：Sheet 内对选中 trigger 节点执行模拟触发，沙盒运行时独立于预览/画布状态。
