...
- **`use-blueprint-preview-runtime.ts` 调度 V1 `interval` 事件**：扫描 V1 编译规则中 `triggerConfig.type === 'interval'` 的规则，按 `intervalMs` 建立 `setInterval`，定期调用 `triggerAndExecute`（V2 蓝图的 `globalType` enum 不包含 `interval`，无需处理）
...
#### Scenario: 组件在编辑器画布中不派发事件
- **WHEN** 组件在编辑器画布中渲染
- **THEN** `useComponentEvent()` 返回 `null`
- **AND** 组件派发逻辑被短路，不触发任何蓝图规则
...
- **WHEN** 组件在编辑器画布中渲染
- **THEN** `useComponentEvent()` 返回 `null`
- **AND** 即使数据源状态变化，也不派发任何事件

#### Scenario: 状态未变化时不重复派发
- **WHEN** 数据源状态从 `success` 因定时刷新再次变为 `success`
- **THEN** 仍派发 `dataLoaded` 事件（每次刷新成功都派发，符合用户配置定时刷新后期望每次刷新都触发动作的语义）

### Requirement: V1 interval 触发器调度

系统 SHALL 在预览运行时为所有 V1 蓝图的 `interval` 触发器建立 `setInterval` 调度。V2 蓝图的 `globalType` enum 不包含 `interval`，无需处理。
...
- **AND** 每次 tick 派发 `{ kind: 'interval' }` 事件，触发所有匹配的 V1 规则
...
新增 V1 `interval` 触发器调度逻辑：
- 扫描 V1 `compiledRules` 中 `triggerConfig.type === 'interval'` 的规则，按 `intervalMs` 建定时器
- 使用 `useEffect` 管理定时器生命周期，规则变化时清理重建
- V2 蓝图不支持 `interval` 触发器（`globalType` enum 不包含 `interval`），无需处理
