...
## 阶段 3：V1 interval 触发器调度

- [x] `use-blueprint-preview-runtime.ts` 新增 `useEffect` 管理 interval 调度
- [x] 仅在 V1 蓝图（`v1CompiledRules.length > 0` 且 `v2CompiledRules.length === 0`）时启用
- [x] 遍历规则过滤 `triggerConfig.type === 'interval'`
- [x] 按 `triggerConfig.intervalMs` 建立 `setInterval`
- [x] tick 回调调用 `triggerAndExecute(v1RulesRef.current, { kind: 'interval' }, depsRef.current)`
...
- [ ] 手动验证：编辑器画布不触发任何蓝图事件（需用户在浏览器中手动验证）
