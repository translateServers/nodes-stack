...
export function useBlueprintClipboard(
  ...
  const notifyError = useCallback(
    (message: string): void => notifications?.notify('error', message),
    [notifications],
  );
  ...
  const paste = useCallback(async () => {
    ...
    try {
      ...
      notifyError('剪贴板内容不是有效的 JSON');
      return;
    }

    const result = BlueprintClipboardSchema.safeParse(json);
    if (!result.success) {
      notifyError('剪贴板内容不是有效的蓝图数据');
      return;
    }
