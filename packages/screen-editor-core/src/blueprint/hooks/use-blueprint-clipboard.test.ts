...
vi.mock('../../components/screen-editor-notifications', () => ({
  useOptionalScreenEditorNotifications: () => ({ notify: mockNotify }),
}));
...
describe('useBlueprintClipboard（任务 5.5）', () => {
  ...
  it('paste：非法 JSON 内容显示错误提示', async () => {
    ...
    expect(mockNotify).toHaveBeenCalledWith('error', '剪贴板内容不是有效的 JSON');
  ...
  it('paste：非蓝图格式的 JSON 显示错误提示', async () => {
    ...
    expect(mockNotify).toHaveBeenCalledWith('error', '剪贴板内容不是有效的蓝图数据');
    expect(setNodes).not.toHaveBeenCalled();
  });
