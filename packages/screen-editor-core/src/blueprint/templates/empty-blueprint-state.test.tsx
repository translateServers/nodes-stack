/**
 * EmptyBlueprintState 组件测试（任务 9.3 / 任务 6.2 V2 适配）
 *
 * 验证点：
 * - 渲染标题与引导文案
 * - 渲染 TemplateGallery（含四个 V2 模板卡片，含 click-delay-show）
 * - 渲染"从空白开始"按钮
 * - 点击模板卡片：校验通过时调用 onInsertTemplate，传入完整 V2 蓝图
...
describe('EmptyBlueprintState（任务 6.2 V2）', () => {
  beforeEach(() => {
  ...
  describe('渲染', () => {
    ...
    it('渲染四个模板卡片（TemplateGallery 子组件，含 click-delay-show）', () => {
      render(
        <EmptyBlueprintState
          onInsertTemplate={vi.fn()}
          onError={vi.fn()}
  ...
  describe('点击模板卡片 - 校验通过路径', () => {
    it('点击 click-navigate 卡片调用 onInsertTemplate，传入完整 V2 蓝图', () => {
      ...
      const arg = onInsertTemplate.mock.calls[0]?.[0] as EventBlueprintV2;
    ...
    it('点击不同模板卡片调用 onInsertTemplate，传入不同 V2 蓝图', () => {
      const onInsertTemplate = vi.fn();
      render(
        <EmptyBlueprintState
          onInsertTemplate={onInsertTemplate}
      ...
      const bp1 = onInsertTemplate.mock.calls[0]?.[0] as EventBlueprintV2;
      const bp2 = onInsertTemplate.mock.calls[1]?.[0] as EventBlueprintV2;

      // click-navigate 源节点为组件节点 A，page-load-refresh 源节点为全局 pageLoad 节点
      const source1 = bp1.nodes[0];
      const source2 = bp2.nodes[0];
      expect(source1?.kind).toBe('component');
      expect(source2?.kind).toBe('component');
      if (source1?.kind === 'component' && source2?.kind === 'component') {
        // click-navigate: 普通组件节点（componentId 为空字符串占位）
        // page-load-refresh: 全局 pageLoad 节点（componentId === 'global', globalType === 'pageLoad'）
        expect(source1.componentId).toBe('');
        expect(source1.globalType).toBeUndefined();
        expect(source2.componentId).toBe('global');
        expect(source2.globalType).toBe('pageLoad');
      }
    });

    it('点击 click-delay-show 卡片调用 onInsertTemplate，传入 3 节点 2 边延时蓝图', () => {
      const onInsertTemplate = vi.fn();
      render(
        <EmptyBlueprintState
          onInsertTemplate={onInsertTemplate}
      ...
      // 中间节点为 delay 节点
      expect(arg.nodes[1]?.kind).toBe('delay');
    });
  });
