...
function RequestTestPanel({
  ...
  const handleTest = () => {
    ...
    const run = async (): Promise<void> => {
      try {
        const response = await dataRuntime.previewApi(
          {
            url,
            method: 'GET',
            headers: getHeaders(),
        ...
        if (!controller.signal.aborted && requestId === requestIdRef.current) {
          setState({
            ...
            preview: truncatePreview(response.data),
          });
          onSampleReceived?.(response.data);
        }
      } catch (error) {
        ...
        setState({
          status: 'error',
          message:
            error instanceof Error ? error.message : '网络请求失败（可能是网络异常或跨域限制）',
        });
      }
    };
...
export function BarChartDataSourceSection({ component, onUpdate }: SectionProps) {
  ...
  const dataRuntime =
    useOptionalScreenEditorRuntimeProfile()?.dataRuntime ??
    DYNAMIC_SCREEN_EDITOR_RUNTIME_FALLBACK.dataRuntime;
  const DatasetConfigForm = dataRuntime.DatasetConfigForm;
  ...
  return (
    <PanelSection title="数据" collapsible testId="datasource-section" contentClassName="space-y-2">
      ...
      {shownType === 'dataset' && DatasetConfigForm !== undefined && (
        <DatasetConfigForm
