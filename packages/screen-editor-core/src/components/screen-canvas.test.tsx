...
function setupStore(
  ...
  const store: Record<string, unknown> = {
    ...
    interactionMode: overrides.interactionMode ?? 'design',
...
describe('画布交互模式与蓝图运行时', () => {
  beforeEach(() => {
    mockUseStore.mockReset();
  });

  it('关闭时不执行事件，开启时应用动作，重新关闭后恢复项目原始状态', async () => {
    const store = setupStore({ selectedComponentIds: ['c1'], interactionMode: 'design' });
    ...
    store.interactionMode = 'interactive';
