 ...
 * - 分布按钮：selectedCount >= 3 时可用（2 个节点时无中间元素可调整，点击为 no-op，故禁用）
 *
 * 交互：
...
export function AlignDistributeToolbar({
  ...
  // 分布需 >=3：2 个节点只有两端锚点、无中间元素可调整，纯函数必返回 hasChange=false，
  // 禁用避免提供"点击无反应"的按钮（与右键菜单 BlueprintContextMenu 的 canDistribute 规则一致）
